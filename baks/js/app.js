...
burger.addEventListener("click", () => {
  ...
  side.classList.toggle("side-minify");
});
...
(function () {
  casino = {
    ...
    zoomAdaptate: function () {
      $(".roullete .line").css(
        "transform",
        `translateX(${$(".roullete")[0].clientWidth / 2}px)`
      );
      $(window).resize(() => {
        $(".roullete .line").css(
          "transform",
    ...
    roulleteSpin: function () {
      let scroll_ = 0;
      let oneCircle = 85;

      scroll_ -= 7 + oneCircle * 0.5;

      $(".roullete .content").css("transform", `translateX(${scroll_}px)`);

      setTimeout(() => {
        $(".roullete .content").css("transition", `3s ease-in-out`);
      }, 100);

      const getColor = () => {
        return $(".roullete .content")
          .children()
          [this.div(-scroll_, oneCircle) + 5].className.slice(5);
      };
      ...
      setInterval(() => {
        this.accessToBet = false;
        $(".roullete .content").css("transition", `.5s ease-in-out`);

        setTimeout(() => {
          update(-oneCircle * num);
        }, 10);
        ...
        setTimeout(() => {
          num = this.getRandomInt(20) + 10;
          update(oneCircle * num);
          console.log(getColor());
        }, 520);

        setTimeout(() => {
          this.accessToBet = true;
          const inter = setInterval(() => {
            $(".timer")[0].innerHTML =
              Math.round((parseFloat($(".timer").html()) - 0.1) * 100) / 100;
            if (parseFloat($(".timer").html()) < 0.1) {
              $(".timer")[0].innerHTML = "20.0";
              clearInterval(inter);
            }
          }, 100);
        }, 3500);
    ...
    otherEvents: function () {
      $(".bet-num").on("click", function (e) {
        if (e.target.innerHTML[0] === "+") {
          $(".bet")[0].value =
            (parseInt($(".bet").val()) || 0) +
            parseInt(e.target.innerHTML.slice(1));
        } else {
          $(".bet")[0].value =
            (parseInt($(".bet").val()) || 0) *
            parseInt(e.target.innerHTML.slice(1));
